...
static int soapysdr_gain_str_set(SoapySDRDevice *dev, char *gain_str)
{
...
#ifdef SOAPYSDR
    if (dev->soapy_dev)
        r = soapysdr_auto_gain(dev->soapy_dev);
#endif
#ifdef RTLSDR
    if (dev->rtlsdr_dev)
...
int sdr_set_tuner_gain(sdr_dev_t *dev, char *gain_str, int verbose)
{
    ...
    /* Enable manual gain */
    if (dev->soapy_dev)
        return soapysdr_gain_str_set(dev->soapy_dev, gain_str);
#endif
#ifdef RTLSDR
